import { User as MongooseUser } from '../entities/mongoose.user.entity';
import { User } from 'src/core/entities/user.entity';
export class MongooseUserMapper {
  static toDomain(entity: MongooseUser): User {
    const model = new User({
      id: entity.id.toString(),
      first_name: entity.first_name,
      last_name: entity.last_name,
      username: entity.username,
      password_hash: entity.password_hash,
      phone_number: entity.phone_number,
      address: entity.address,
      role: entity.role,
      deleted_at: entity.deleted_at,
    });
    return model;
  }
  static toMongoose(entity: User): MongooseUser {
    const user = new MongooseUser();
    user.first_name = entity.first_name;
    user.last_name = entity.last_name;
    user.username = entity.username;
    user.password_hash = entity.password_hash;
    user.phone_number = entity.phone_number;
    user.address = entity.address;
    user.role = entity.role;
    user.deleted_at = entity.deleted_at;
    return user;
  }
  static toCreateMongoose(entity: User): MongooseUser {
    const user = new MongooseUser();
    user.first_name = entity.first_name;
    user.last_name = entity.last_name;
    user.username = entity.username;
    user.password_hash = entity.password_hash;
    user.phone_number = entity.phone_number;
    user.address = entity.address;
    user.role = entity.role;
    return user;
  }
}
